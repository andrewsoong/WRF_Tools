   ...
   DO i = 1 , num_obs
      ...
      IF (   out_opt .EQ. 0                                   .OR. &
         ...
         IF ((obs(i)%info%platform(1:11) == 'FM-88 SATOB') .OR. &
           ...
           !BPR BEGIN
           !If this ob has just switched designation from not being a sounding
           !to being a sounding, true_num_obs=1, but for soundings it should be
           !true_num_obs=0 at this point
           true_num_obs=0
           !BPR END
         ENDIF
        ...
        !is_sounding=FALSE and either 1) the ob height does not match the station
        !elevation height, or 2) both heights are missing, then 
        !without the following code discard might be false and
        ...
        IF(.NOT. is_sounding) THEN 
         ...
         ELSEIF(eps_equal( obs(i)%info%elevation, missing_r, 1.) ) THEN
          !Note that this ELSEIF will only execute when both the ob height and
          !station elevation height are both missing.  If only one is missing then
          !the IF above will be true since they will not be equal to one another
          obs(i)%info%discard = .TRUE.
          PRINT '(A,A,A,A,A,A,A,A,F12.2,A,F12.2,A,A,A,A)',&
           'WARNING: Ob indicates is_sounding=.FALSE. ',&
           'but the ob elevation is missing.  This ob will be omitted since Obsgrid assumes ',&
           'is_sounding=.FALSE. indicates a surface ob, but without an elevation a sfc ob cannot ',&
           'be used. This ob may actually be a non-surface ob and is being unnecessarily removed. ',&
           'If this is the case mark this ob as a sounding in the input file even if it is not ',&
           'truly a sounding since Obsgrid assumes all non-surface obs should be marked as ',&
           'soundings',&
           '. Height = ', next%meas%height%data, &
           ', Elevation = ', obs(i)%info%elevation,', Ob ID = ',TRIM(ADJUSTL(obs(i)%location%id)),&
           ', Ob Time = ', obs(i)%valid_time%date_char
         ENDIF
        ENDIF
